...
function hotkeys:init(args)
    ...
    { {}, "s", {}, {} }, -- switch window group
    { {}, "m", {}, {} }, -- move to window group
	}
  ...
  keyseq[3][7][3] = {
      {
          {}, "1", function() awful.screen.focus(1) end,
          { description = "Switch to first screen", group = "Layouts", keyset = { "1" } }
      },
      {
          {}, "2", function() awful.screen.focus(2) end,
          { description = "Switch to second screen", group = "Layouts", keyset = { "2" } }
      },
      {
          {}, "s", function() awful.screen.focus(awful.screen.focused().index == 1 and 2 or 1) end,
          { description = "Switch to next screen", group = "Layouts", keyset = { "s" } }
      },
  }

  keyseq[3][8][3] = {
      {
          {}, "1", function() client.focus:move_to_screen(1) end,
          { description = "Move client to screen 1", group = "Layouts", keyset = { "1" } }
      },
      {
          {}, "2", function() client.focus:move_to_screen(2) end,
          { description = "Move client to screen 2", group = "Layouts", keyset = { "2" } }
      },
      {
          {}, "m", function() client.focus:move_to_screen() end,
          { description = "Move client to next screen", group = "Layouts", keyset = { "m" } }
      },
  }
	-- Layouts
	--------------------------------------------------------------------------------
	...
	self.raw.root = {
		...
		{
			{ env.mod }, "Return", function() awful.spawn(env.terminal, {opacity=0.2}) end,
	...
	self.raw.client = {
		...
		{
				{ env.mod }, "Print", function () awful.util.spawn("scrot -e 'mv $f ~/Pictures/screenshots/ 2>/dev/null' ", false) end,
				{ description = "take screenshots", group = "Client Keys"}
		},
	}
